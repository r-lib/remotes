...
* Add `remote_precedence` parameter to `dev_package_deps()` and `combine_remote_deps()` that allows
  user to choose whether remote sources should have a priority over cran sources. Parameter defaults
  to TRUE for backward compatibility reasons. (#732, @maksymiuks)
