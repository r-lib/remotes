...
Remotes, just like devtools, supports package dependency installation
for packages not yet in a standard package repository such as
...
`DESCRIPTION` file. This will cause remotes to download and install them
...
if you want a particular commit. Otherwise the latest commit on the
...
be available on CRAN. For this reason, `devtools::release()` will warn
you if you try to release a package with a `Remotes` field.
