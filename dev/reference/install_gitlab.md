...
packages in a single command. Like other remotes the repository will
skip installation if `force == FALSE` (the default) and the remote state
has not changed since the previous installation.
...
- auth_token:
  ...
  with at least read_api scope in
  <https://docs.gitlab.com/ee/user/profile/personal_access_tokens.html>
  and supply to this argument. This is safer than using a password
  because you can easily delete a PAT without affecting any others.
  Defaults to the GITLAB_PAT environment variable.

- host:

  GitLab API host to use. Override with your GitLab enterprise hostname,
  for example, `"<PROTOCOL://>gitlab.hostname.com"`. The PROTOCOL is
  required by packrat during Posit Connect deployment. While
  install_gitlab may work without, omitting it generally leads to
